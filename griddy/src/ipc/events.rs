    ...
    pub fn format(&self) -> String {
        let (name, data): (&str, String) = match self {
            ...
            Event::WorkspaceRenamed { col, row, old, new } => (
                "workspace_renamed",
                format!("{col},{row},winit,{old},{new}"),
            ),
            Event::WorkspaceSyncChanged { synced } => (
                "workspace_sync_changed",
                if *synced { "synced" } else { "unsynced" }.into(),
            ),
            Event::WindowOpened {
                id,
                app_id,
                ...
                "window_opened",
                format!("{id},{app_id},{workspace_col},{workspace_row},{state},{slot}"),
            ),
            Event::WindowClosed { id } => ("window_closed", id.to_string()),
            Event::WindowMoved { id, col, row } => ("window_moved", format!("{id},{col},{row}")),
            Event::WindowPlaced {
                id,
                policy_matched,
                slot_assigned,
                ...
                "window_placed",
                format!("{id},{policy_matched},{slot_assigned}"),
            ),
            Event::WindowFocus { id } => ("window_focus", id.to_string()),
            Event::WindowTitle { id, title } => ("window_title", format!("{id},{title}")),
            Event::WindowAppId { id, app_id } => ("window_app_id", format!("{id},{app_id}")),
            ...
            Event::WindowSlotAdapted {
                id,
                requested,
                actual,
            } => ("window_slot_adapted", format!("{id},{requested},{actual}")),
            Event::WindowStateAdapted {
                id,
                requested_state,
                actual_state,
                actual_slot,
            } => (
                "window_state_adapted",
                format!("{id},{requested_state},{actual_state},{actual_slot}"),
            ),
            Event::WindowMoveSkipped {
                id,
                requested_col,
                requested_row,
                actual_col,
                ...
                "window_move_skipped",
                format!("{id},{requested_col},{requested_row},{actual_col},{actual_row}"),
            ),
            Event::WindowMoveRefused {
                id,
                requested_col,
                requested_row,
                reason,
                ...
                "window_move_refused",
                format!("{id},{requested_col},{requested_row},{reason}"),
            ),
            ...
            Event::SubMapChanged { name } => ("submap_changed", name.clone()),
            Event::ConfigReloaded { which } => ("config_reloaded", which.clone()),
            Event::ConfigError { message } => ("config_error", message.clone()),
            Event::ThemeReloaded => ("theme_reloaded", String::new()),
            ...
            Event::OverviewOpened => ("overview_opened", String::new()),
            Event::OverviewClosed => ("overview_closed", String::new()),
            Event::ViewModeChanged { mode } => ("view_mode_changed", mode.clone()),
            Event::MinimapToggled { visible } => ("minimap_toggled", visible.to_string()),
            Event::MonitorAdded {
                name,
                width,
                height,
            ...
            Event::MonitorRemoved { name } => ("monitor_removed", name.clone()),
            Event::MonitorConfigChanged { name } => ("monitor_config_changed", name.clone()),
            Event::KeyboardLayoutChanged { device, layout } => {
                ("keyboard_layout_changed", format!("{device},{layout}"))
            }
            Event::IdleTimeout { after_seconds } => ("idle", after_seconds.to_string()),
            Event::IdleResume { after_seconds } => ("resume", after_seconds.to_string()),
            Event::Urgent { id } => ("urgent", id.to_string()),
            Event::SessionLocked => ("lock", String::new()),
            Event::SessionUnlocked => ("unlock", String::new()),
            Event::SafeModeEntered { reason } => ("safe_mode_entered", reason.clone()),
            Event::NotificationDaemonMissing => ("notification_daemon_missing", String::new()),
            Event::PluginError { name, message } => ("plugin_error", format!("{name},{message}")),
            Event::PluginLoaded { name } => ("plugin_loaded", name.clone()),
            Event::WindowSizeConstraintFloat {
                id,
                slot,
                min_w,
                min_h,
...
mod tests {
    ...
    fn safe_mode_entered_wire() {
        let wire = Event::SafeModeEntered {
    ...
    fn config_error_wire() {
        let wire = Event::ConfigError {
            message: "parse error on line 5".into(),
        }
        .format();
        let (name, data) = parse(&wire);
