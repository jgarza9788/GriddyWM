...
impl BindTable {
    ...
    pub fn for_cheatsheet(&self) -> Vec<(String, String, String, bool, bool)> {
        let mut out: Vec<(String, String, String, bool, bool)> = self
            .entries
            .iter()
            .map(|e| {
    ...
    pub fn lookup_flags(
        ...
        sym: u32,
        mods: &ModifiersState,
    ) -> Option<(Action, bool, bool, bool, bool)> {
...
impl GestureTable {
    pub fn from_config(gestures: &[crate::config::types::GestureConfig]) -> Self {
        let mut entries = Vec::new();
        for g in gestures {
            if g.action.is_empty() || g.direction.is_empty() {
                continue;
            }
    ...
    pub fn from_defaults() -> Self {
        ...
        Self::from_config(&[
            ...
            GestureConfig {
                fingers: 3,
                direction: "left".into(),
                action: "workspace-right".into(),
                args: vec![],
            },
            GestureConfig {
                fingers: 3,
                direction: "right".into(),
                action: "workspace-left".into(),
                args: vec![],
            },
        ])
    }
