...
impl CompositorHandler for GlobalState {
    ...
    fn commit(&mut self, surface: &WlSurface) {
        ...
        if let Some(wid) = window_id {
            ...
            let (prev_app_id, prev_title) = self.grid.windows.get(&wid)
                .map(|w| (w.app_id.clone(), w.title.clone()))
                .unwrap_or_default();

            self.grid.update_window_metadata(wid, app_id.clone(), title.clone());

            // Emit change events when title or app_id actually change.
            if !title.is_empty() && title != prev_title {
                self.pending_events.push(crate::ipc::events::Event::WindowTitle {
                    id: wid, title: title.clone(),
                });
            }
            if !app_id.is_empty() && app_id != prev_app_id {
                self.pending_events.push(crate::ipc::events::Event::WindowAppId {
                    id: wid, app_id: app_id.clone(),
                });
            }

            // Keep ext-foreign-toplevel-list-v1 handle in sync.
            if let Some(handle) = self.toplevel_handles.get(&wid) {
                if !app_id.is_empty() { handle.send_app_id(&app_id); }
                if !title.is_empty()  { handle.send_title(&title); }
                handle.send_done();
            }
            ...
            if matches!(
                ...
                if min_w > 0 || min_h > 0 {
                    ...
                    let is_tiled = self.grid.windows.get(&wid)
                    ...
                    if is_tiled {
                        ...
                        if let Some(rect) = slot_rect {
                            ...
                            if too_narrow || too_short {
                                let slot_name = self.grid.windows.get(&wid)
